...
Capabilities are grouped by functional area. For each capability, the entry specifies what it is, why it is required, and the maturity stage at which the organization cannot proceed without it. Maturity stages referenced throughout are: **Foundation** (first agents in sandboxed environments), **Operational** (agents operating in production with real data and tool access), **Optimized** (enterprise-wide agent deployment with agent-to-agent communication and complex delegation chains).
...
**Critical maturity stage:** **Foundation.** The registry must exist before the first agent is deployed beyond a local development environment. Even a single-agent pilot must be registered so that identity, ownership, and access controls are established from the outset.
...
**Critical maturity stage:** **Foundation.** Ownership binding must be enforced from the first agent registration. Deferring it creates orphaned agents that are difficult to remediate retroactively.
...
**Critical maturity stage:** **Operational.** During the Foundation stage, agents are few enough that lifecycle can be managed informally. As the agent population grows in the Operational stage, formal state management with enforced transitions becomes essential.
...
**Critical maturity stage:** **Operational.** During the Foundation stage in sandboxed environments, credential management can be simplified. Once agents access production systems with real data in the Operational stage, just-in-time credential issuance with expiration becomes a hard requirement.
...
**Critical maturity stage:** **Operational.** Per-invocation authentication adds overhead that may be deferred during sandboxed Foundation experiments. In the Operational stage, where agents access production tools, per-invocation authentication is required to maintain traceability and limit exposure.
...
**Critical maturity stage:** **Optimized.** Agent-to-agent communication typically emerges at scale when organizations build multi-agent architectures. Mutual authentication must be in place before agent-to-agent patterns are approved for production use.
...
**Critical maturity stage:** **Operational.** During the Foundation stage, a standalone agent registry may suffice. In the Operational stage, when agents access the same production systems as human users, integration with the corporate identity provider becomes necessary for consistent enforcement and visibility.
...
**Critical maturity stage:** **Foundation.** Even the first agent must have an explicit tool authorization list. Deploying an agent with unrestricted tool access, even in a sandbox, establishes a dangerous precedent.
...
**Critical maturity stage:** **Operational.** During the Foundation stage, tool-level controls may suffice for simple, single-tool agents. As agents access production data and multi-tool workflows emerge in the Operational stage, parameter-level enforcement becomes critical to prevent over-privileged tool usage.
...
**Critical maturity stage:** **Optimized.** Delegation-chain enforcement is required when multi-agent workflows and agent-to-agent delegation patterns are deployed in production. Single-agent architectures in the Foundation and Operational stages may defer this capability.
...
**Critical maturity stage:** **Operational.** Basic logging may suffice in the Foundation stage. In the Operational stage, when agents act on production data, a complete, structured, SIEM-integrated audit trail is a hard requirement for security operations and compliance.
...
**Critical maturity stage:** **Operational.** Delegation logging must be in place when agents move to production. For multi-agent delegation patterns in the Optimized stage, the logging must support chain reconstruction across agent boundaries.
...
**Critical maturity stage:** **Operational.** During the Foundation stage, the agent population is small enough that manual tracking may suffice. In the Operational stage, as more teams adopt agent patterns, the probability of unregistered agents increases significantly.
...
**Critical maturity stage:** **Optimized.** During the Foundation and Operational stages, agent populations are small enough for manual review. At the Optimized stage, automated detection becomes necessary to maintain least privilege across a large agent population.
...
**Critical maturity stage:** **Optimized.** During the Foundation and Operational stages, the tool surface is typically small and manually managed. At the Optimized stage, tool endpoints proliferate across teams, and automated discovery becomes necessary to maintain a complete inventory.
...
**Critical maturity stage:** **Operational.** Formal periodic reviews are required once agents access production systems. During the Foundation stage, ad-hoc review is acceptable.
...
**Critical maturity stage:** **Optimized.** During the Operational stage, compliance evidence can be assembled manually from audit logs. At the Optimized stage, automated reporting is required to meet the volume and frequency of compliance obligations.
